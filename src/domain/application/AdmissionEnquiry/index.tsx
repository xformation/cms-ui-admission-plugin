...
class AdmissionEnquiry<T = {[data: string]: any}> extends React.Component<AdmissionEnquiryProps, any> {
    ...
    registerSocket() {
        ...
        socket.onopen = () => {
           console.log("1. Opening websocekt connection on Admission index.tsx. User : ",this.state.user);
            ...
            <section className="tab-container row vertical-tab-container">
                ...
                <TabContent activeTab={activeTab} className="col-sm-9 border-left p-t-1">
                    <TabPane tabId={0}>
                        {
                            user !== null && (
                                <AdmissionEnquiryPage user={user} operationType={"ADD"} ></AdmissionEnquiryPage>
                            )
                        }
                    </TabPane>
                    <TabPane tabId={1}>
                        {
                            user !== null && enquiryList !== null && (
                                <EnquiryGrid user={user} type="Total Received" totalRecords={enquiryList.getAdmissionEnquiryList.length} data={enquiryList.getAdmissionEnquiryList}></EnquiryGrid>
                            )
                        } 
                    </TabPane>
                    <TabPane tabId={2}>
                        {
                            user !== null && enquiryList !== null && (
                                <EnquiryGrid user={user} type="Total Follow Up" totalRecords={enquiryList.getAdmissionEnquiryList.length} data={enquiryList.getAdmissionEnquiryList}></EnquiryGrid>
                            )
                        } 
                    </TabPane>
                    <TabPane tabId={3}>
                        {
                            user !== null && enquiryList !== null && (
                                <EnquiryGrid user={user} type="Total Declined" totalRecords={enquiryList.getAdmissionEnquiryList.length} data={enquiryList.getAdmissionEnquiryList}></EnquiryGrid>
                            )
                        } 
                    </TabPane>
                    <TabPane tabId={4}>
                        {
                            user !== null && enquiryList !== null && (
                                <EnquiryGrid user={user} type="Total Admission Granted" totalRecords={enquiryList.getAdmissionEnquiryList.length} data={enquiryList.getAdmissionEnquiryList}></EnquiryGrid>
                            )
                        } 
                    </TabPane>
                    <TabPane tabId={5}>
                        {
                            user !== null && enquiryList !== null && (
                                <EnquiryGrid user={user} type="Total Enquiries" totalRecords={enquiryList.getAdmissionEnquiryList.length} data={enquiryList.getAdmissionEnquiryList}></EnquiryGrid>
                            )
                        }                        
                    </TabPane>
                </TabContent>
